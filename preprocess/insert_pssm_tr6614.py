...
def create_new_encoding(pmap, pssm_data):
    ...
    for i in range(len(pmap["X"])):
        ...
        for j, (x, y) in enumerate(zip(primary_structure, secondary_structure)):
            ...
            pssm_and_conservation = [float(0) for v in list(pssm_data[i][j])]
...
# %%
print('ZEROD PSSM')
pssm_data = np.load("../experiments/1204_090945/tr6614_whole_gen_pssm.npy")
formatted_data = create_new_encoding(pmap, pssm_data)


# %%

with open('../data/TR6614_aug_transformer_v3_whole_zero_pssm.json', 'w') as outfile:
